...
![](/home/gianluca/Scrivania/methyl.O/vignettes/logo6.png)
...
<img src="/home/gianluca/Scrivania/methyl.O/vignettes/genemodels.png" alt=" **Figure 1**: Schematic gene model " width="90%" height="90%">
...
```R
annotatedDMRs <- annotateDMRs(DMRsSubset , prom.length=1500, head.length=1500, 
longest.trx=TRUE, annotation='ensembl', hg='hg19', annotation.fast=TRUE, 
thr.beta=.3, thr.cgis=.4, col.betadiff = 4, col.beta1 = NULL, col.beta2 = NULL)
```
...
```R
annotatedDMRs <- scoreAnnotatedDMRs(annotatedDMRs, 
active.features = c("promoters", "heads"), score.modifier = 0.5)
```
...
```R
annotatedEnhancers <- annotateEnhancers(DMRsSubset, hg='hg19', thr.beta = 0.3, 
overlap.param.thr = 40,  param.type = "overlap.percentage", 
score.modifier = 0.5, col.betadiff = 4)
```
...
![**Figure 2**: Starting from the top of the plot are shown: the chromosome location, the CGIs ranges, the promoter range and its direction, one or more transcripts associated with the gene and the information about the DMR including the beta difference value and length.  ](/home/gianluca/Scrivania/methyl.O/vignettes/visualizeMethylation.png)
...
```R
associateFeat2Exprs(annotatedDMRs, expressionSubset, 
active.features = c("promoters", "heads"), col.genes =1 , 
col.stat = 6, stat.thr = 0.05, col.logFC = 2, logfc.thr = 0, 
beta.thr = .3, plot.type = 'splitted', cor.type = 'pearson', 
return.table = FALSE)
```
...
![**Figure 4**: The plot shows on the left the beta Difference of Methylation between two compared conditions of specific TFs, STAT1 in this case, and the expression of target genes. ](/home/gianluca/Scrivania/methyl.O/vignettes/EnrichrTFsPlot.png)
...
```R
TFs2Targets <- associateTFs2Exprs(annotatedDMRs, expressionSubset,
active.features = c("promoters", "heads") col.genes = 1, col.stat = 6, 
stat.thr = 0.05, col.logFC = 2, logfc.thr = 0, beta.thr = .3, 
