...
#'  
#' @param annotatedResults anotated results list from getMetAnnotations()
#' @param active.features annotation level from the gene symbols are taken. Default = c('promoters', 'heads') 
#' @param stat.filter character string indicating which type of statistics use for filtering results. Accepted values: 'P.value', 'Adjusted.P.value' or 'Overlap'. Default 'P.value'.
...
annotatedDMRs2Enrichr <- function(annotatedResults, active.features = c('promoters', 'heads'), stat.filter = 'P.value', stat.thr = 0.01, db=NULL){
    ...
    for(af in active.features){
        query.vector <- c(query.vector, annotatedResults[[af]]$symbol)
    } 
