...
#' 
#' Query targeted genes from TF in order to find enriched proceses.
#'  
#' @param associated.table data.frame. Corresponding to resulting data.frame from tfExprsCor().
#' @param logfc.thr value indicating logFC threshold 
#' @param stat.filter character string indicating which type of statistics use for filtering results. Accepted values: 'P.value' or 'Adjusted.P.value'. Default 'P.value'.
...
tfs2Enrichr <- function(associated.table, logfc.thr = 1, stat.filter = 'P.value', stat.thr = 0.01, db = NULL){
    if(!any((.packages()) %in% "enrichR")){
        library(enrichR)
    }

    associated.table <- associated.table[abs(associated.table$target.expression)>=logfc.thr,]
    ...
    enirchr.results <- enrichR::enrichr(as.character(associated.table$target.symbol), databases = manifest)
