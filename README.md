...
author: 
- name: Gianluca Mattei
  email: gianluca.mattei@unifi.it
- Federico Di Salvi
- Alberto Magi
package: Methyl.O
output: 
...
```GitHub
devtools::install_github(gmattei/Methyl.O)
```
...
![**Figure 1**: Schematic gene model](/home/gianluca/Scrivania/methylO/vignettes/genemodels.pdf)
: (\#fig:figure1)
...
starts <-c(x0d, x0p, x0e1, x0i1, x0e2, x0i2, x0e3, x0i4, x0e5, x0i5, x0e6, x0i6, x0e7, x0d2)

ends <-c( x1d, x1p, x1e1, x1i1, x1e2, x1i2, x1e3, x1i4, x1e5, x1i5, x1e6, x1i6, x1e7, x1d2)

col = c('black',"green", rep(c("#ffff00","#d7d7d7"),5),"#ffff00",'black')

par(mar=c(0,0,0,0))
plot(x=c(-5,70), y =c(0,8), xaxt = 'n', yaxt = 'n', bty = 'n', pch = '', ylab = '', xlab = '')
y0 <- 6
y1 <- 6.2
rect(xleft = starts, ybottom = y0, xright = ends, ytop=y1, col=col )

# 5'utr singolo
shift <- .25
rect(xleft = starts[3], ybottom = (y0 + shift), xright = ends[3], ytop=y1 + shift, col= c("#ff0000"), border = c("black") )

# 3' singolo
rect(xleft = starts[13], ybottom = (y0 + shift), xright = ends[13], ytop=y1 + shift, col= c("#ff0000"), border = c("black") )

# tss
rect(xleft = starts[2], ybottom = (y0 - shift *2), xright = ends[5], ytop=y1 - shift * 2, col= "#3cbfeb", border = "black" )

# HEAD
rect(xleft = starts[3], ybottom = (y0 - shift ), xright = ends[5], ytop=y1 - shift , col= "#ff0080", border = "black" )

y0b <- 4
y1b <- 4.2
rect(xleft = starts, ybottom = y0b, xright = ends, ytop=y1b, col=col )

# 5'utr singolo
shift <- .25
rect(xleft = starts[3], ybottom = (y0b + shift), xright = ends[3], ytop=y1b + shift, col= c("#ff0000"), border = c("black") )

# tss
rect(xleft = starts[2], ybottom = (y0b - shift *2), xright = ends[5], ytop=y1b - shift * 2, col= "#3cbfeb", border = "black" )

# HEAD
rect(xleft = starts[3], ybottom = (y0b - shift ), xright = ends[5], ytop=y1b - shift , col= "#ff0080", border = "black" )

y0c <- 2
y1c <- 2.2

rect(xleft = starts, ybottom = y0c, xright = ends, ytop=y1c, col=col )

# 5'utr doppio
shift <- .25
rect(xleft = starts[c(3,5)], ybottom = (y0c + shift), xright = ends[c(3,5)], ytop=y1c + shift, col= c("#ff0000",  "#ff0000"), border = c("black",  "black") )

# 3' doppio
rect(xleft = starts[c(11,13)], ybottom = (y0c + shift), xright = ends[c(11,13)], ytop=y1c + shift, col= c("#ff0000", "#ff0000"), border = c("black", "black") )

# tss
rect(xleft = starts[2], ybottom = (y0c - shift *2), xright = ends[5], ytop=y1c - shift * 2, col= "#3cbfeb", border = "black" )

# HEAD
rect(xleft = starts[3], ybottom = (y0c - shift ), xright = ends[5], ytop=y1c - shift , col= "#ff0080", border = "black" )

legend("bottom", pch=22, pt.bg = c("black", "green","#ffff00", "#d7d7d7", "#ff0000", "#ff0080", "#3cbfeb"),
  legend = c("DNA", "Promoter", "Exons", "Introns", "UTRs", "Head", "TSS.Surrounding"),
  horiz=FALSE, cex = 1, ncol = 2, box.col="white"
)

text(x = -3, y=c(6.5,4.5,2.5), c("A", "B", "C"))
```

```{r, out.width='25%', fig.align='center', fig.cap='...'}
knitr::include_graphics('/home/gianluca/Scrivania/methylO/vignettes/genemodels.pdf')
```
...
![**Figure 4**: The plot shows on the left the beta Difference of Methylation between two compared conditions of specific TFs, STAT1 in this case, and the expression of target genes. ](/home/gianluca/Scrivania/methylO/vignettes/EnrichrTFsPlot.pdf)
